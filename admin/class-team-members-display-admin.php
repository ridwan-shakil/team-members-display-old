...
class Team_Members_Display_Admin {
	...
	public function enqueue_styles( $screen ) {
		...
		if ( 'team_member_display_page_team-member-display-settings' == $screen ) {
			...
			wp_enqueue_style( 'tm-public-style', plugin_dir_url( __FILE__ ) . '../public/css/team-members-display-public.css' );
	...
	function team_display_columns( $columns ) {
		$columns = array(
	...
	function team_display_manage_columns( $column, $post_id ) {
		global $post;
	...
	function save_team_display_meta_fields( $post_id ) {
	...
	// ==============================
	function team_m_display_submenu_page() {
		add_submenu_page(
	...
	function team_member_display_settings_page() {
		require TEAM_PLUGIN_DIR_PATH . 'admin/partials/style-settings.php';
	}

	// add settings fields
	function team_member_display_settings_fields() {
