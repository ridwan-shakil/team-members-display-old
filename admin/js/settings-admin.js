; (function ($) {
    $(document).ready(function () {
        ...
        //     e.preventDefault();
        //     alert('wait for tab to be visible');
        ...
        // ========= Load settings changes instently ===========
        // ------------------------
        //  container section 
        // ------------------------
        // Team column layout
        $('#tms-columns-select').on('change', function () {
        ...
        // member image height
        $('input[name="tms_member_image_height"]').on('input', function () {
            var input = $(this).val();
            $('img.tm-s-image').css('height', input + 'px');
        });
        // Name font size
        $('input[name="tms_name_font_size"]').on('input', function () {
            var input = $(this).val();
            $('h3.tm-s-name').css('font-size', input + 'px');
        });
        // Designation font size
        $('input[name="tms_designation_font_size"]').on('input', function () {
            var input = $(this).val();
            $('p.tm-s-position').css('font-size', input + 'px');
        });
        // Description font size
        $('input[name="tms_description_font_size"]').on('input', function () {
            var input = $(this).val();
            $('p.tm-s-description').css('font-size', input + 'px');
        });
        // Social profile icon size
        $('input[name="tms_social_profile_icon_size"]').on('input', function () {
            var input = $(this).val();
